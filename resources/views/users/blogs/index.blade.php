    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          @if(Auth::user()->email_verified == 0)
            <p class="alert alert-warning">Almost there! <a href="javascript:voic(0)" data-toggle="modal" data-target="#verifymodal">Verify your email</a> to begin uploading.</p>
          @endif
          @if($data['count'] > 0)
            <p class="alert alert-warning">Please note: Only one article can be submitted within a 30-day period.</p>
          @endif
      ...
      <div class="row">
        <div class="col-12">
              ...
              <div class="row">
                ...
                <div class="col-md-3">
                  @if($data['is_eligible'] == 1)
                    <a href="javascript:void(0)" class="btn btn-primary pull-right addblogButton" title="Add Blog"><i class="fas fa-plus"></i> Add Article</a>
                  @endif
                </div>
              </div>
            </div>
          ...
          <div class="card">
            <div class="card-body">
                <h4 class="text-primary">Guest Posting Guidelines</h4>
                <p>To maintain the quality of our website, we have a few guest posting rules that all contributors must follow:</p>
                <ol>
...
<div class="modal fade" id="addBlogFormModal">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="add_blog_form" action="{{route('user.blog.create')}}">
        @csrf
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="hidden" class="blogMetaTitle" name="meta_title" required>
              </div>
            </div>
          </div>
...
<script>
  ...
  $(function() {
    ...
    $(document).on('keyup', '.searchBlog', function() {
      ...
      var url = "{{URL::to('/user/articles/search')}}/" + val;
    ...
    $(document).on('click', '.deleteBlog', function() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.get("{{URL::to('/user/articles/delete')}}/" + id, function(data) {
            Toast.fire({
              icon: 'success',
    ...
    $(document).on('click', '.editBlog', function() {
      ...
      $.get("{{URL::to('/user/articles/edit')}}/" + id, function(data) {
  ...
  function loadBlogs() {
    ...
    $pu = ($pu == 0 ? '' : '?page='.$pu);
    @endphp
    var url = "{{route('user.blog.load').$pu}}";
