          ...
          <div class="card">
            <div class="card-header">
              <div class="row">
                ...
                <div class="col-md-3">
                  <a href="javascript:void(0)" class="btn btn-primary pull-right addblogButton" title="Add Blog"><i class="fas fa-plus"></i> Add Episode</a>
                </div>
              </div>
            </div>
          </div>
...
<div class="modal fade" id="addBlogFormModal">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="add_blog_form" action="{{route('admin.episodes.create')}}">
        @csrf
          ...
          <div class="row">
            ...
            <div class="col-md-4 blog-img-alt">
              <div class="form-group">
                ...
                <input type="text" class="form-control" name="image_alt" required>
              </div>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="link" class="form-control" name="visit_link" required>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  $(function() {
    ...
    $(document).on('keyup', '.searchBlog', function() {
      ...
      var url = "{{URL::to('/admin/panel/episodes/search')}}/" + val;
    ...
    $(document).on('click', '.deleteEpisode', function() {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.get("{{URL::to('/admin/panel/episodes/delete')}}/" + id, function(data) {
            Toast.fire({
              icon: 'success',
              title: 'Success! Episode Successfully Deleted.'
            });
            loadBlogs();
          });
        }
    ...
    $(document).on('click', '.editBlog', function() {
      ...
      $.get("{{URL::to('/admin/panel/episodes/edit')}}/" + id, function(data) {
  ...
  function loadBlogs() {
    ...
    $pu = ($pu == 0 ? '' : '?page='.$pu);
    @endphp
    var url = "{{route('admin.episodes.load').$pu}}";
