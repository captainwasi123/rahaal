<?php

namespace App\Http\Controllers\admin;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Models\Map;
use App\Models\MapDetails;
use App\Models\Blogs;
use App\Models\Episodes;
use App\Models\Playlists;
use App\Models\Categories;
use App\Models\Countries;
use App\Models\Faq;
use App\Models\Author;
use App\Models\MetaTags;
use App\Models\BlogTags;
use App\Models\TagData;
use Auth;

class MapController extends Controller
{
    public function index()
    {
        $data['menu'] = 'episodes';
        $data['data'] = Map::orderBy('created_at', 'desc')->paginate(10);

        return view('admin.map.index')->with($data);
    }

    public function load()
    {
        $p = 1;
        if (!empty($_GET['page'])) {
            $p = $_GET['page'];
        }
        $data = Map::orderBy('created_at', 'desc')->paginate(10, ['*'], 'page', $p);

        return view('admin.map.load', ['data' => $data]);
    }

    public function search($val)
    {
        $response = [];
        $data = Episodes::when($val !== '--empty--', function ($q) use ($val) {
            return $q->where('title', 'like', '%' . $val . '%');
        })->get();

        return view('admin.episodes.load', ['data' => $data]);
    }

    public function create(Request $request)
    {
        $data = $request->all();
        $response = [];


        $blog = Map::where('lat', $data['lat'])->where('lng', $data['lng'])->get();

        if (count($blog) == 0) {

            $id = Map::create($data);


            if ($request->hasFile('coupon_image')) {
                $file = $request->file('coupon_image');
                $ext = $file->getClientOriginalExtension();
                $newname = $id . date('dmyhis') . '.' . $ext;

                $file->move(public_path() . '/storage/map', $newname);

                $b = Map::find($id);
                $b->cover_img = $newname;
                $b->save();
            }

            if ($request->hasFile('album')) {

                foreach ($request->file('album') as $file) {

                    $ext = $file->getClientOriginalExtension();
                    $newname = $id . '_' . time() . '_' . uniqid() . '.' . $ext;

                    $file->move(public_path('storage/map/album'), $newname);

                    // Save each image record
                    $image = new MapDetails(); // replace with your model
                    $image->map_id = $id;
                    $image->img = $newname;
                    $image->save();
                }
            }

            $response['success'] = 'success';
            $response['message'] = 'Success! New Location Added.';
        } else {

            $response['success'] = false;
            $response['errors'] = 'Erorr, Location already exist.';
        }

        echo json_encode($response);
    }

    public function update_blog(Request $request)
    {
        $data = $request->all();
        $response = [];

        if (empty($data['visit_link']) || empty($data['title']) || empty($data['image_alt']) || empty($data['playlist_id'])) {
            $response['success'] = false;
            $response['errors'] = 'Please Fill all required fields.';
        } else {

            $id = Episodes::episode_update(base64_decode($data['episode_id']), $data);


            if ($request->hasFile('edit_mblog_image')) {
                $file = $request->file('edit_mblog_image');
                $ext = $file->getClientOriginalExtension();
                $newname = $id . date('dmyhis') . '.' . $ext;

                $file->move(public_path() . '/storage/episodes', $newname);

                $b = Episodes::find($id);
                $b->image = $newname;
                $b->save();
            }

            $response['success'] = 'success';
            $response['message'] = 'Success! Episode Successfully Updated.';
        }

        echo json_encode($response);
    }


    public function edit($id)
    {
        $id = base64_decode($id);

        $data = Episodes::find($id);
        $data['playlists'] = Playlists::get();


        return view('admin.episodes.edit', ['data' => $data]);
    }



    public function delete($id)
    {
        $id = base64_decode($id);

        Episodes::destroy($id);

        $response = 'success';

        return $response;
    }
}
